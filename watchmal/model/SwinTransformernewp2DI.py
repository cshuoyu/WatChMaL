...
class ConvStem(nn.Module):
    def __init__(
        ...
        layers += [nn.Conv2d(c_in, c_out, 3, 2, 1, bias=True), act_layer()]  # ↓×2
        c_in = c_out
        layers += [
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_windows = window_partition(shifted_x, self.window_size)                 # (BnW, ws, ws, C)
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)   # (BnW, ws*ws, C)
...
class CrossWindowBlock(nn.Module):
    def __init__(self, dim, input_resolution, window_size=4, num_heads=6,
                 mlp_ratio=4.0, drop=0.0, attn_drop=0.0, shift_size=0):
        super().__init__()
        ...
        if self.shift_size > 0:
            H, W = self.HW
            img_mask = torch.zeros((1, H, W, 1))
            h_slices = (slice(0, -self.ws),
                        slice(-self.ws, -self.shift_size),
                        slice(-self.shift_size, None))
            w_slices = (slice(0, -self.ws),
                        slice(-self.ws, -self.shift_size),
                        slice(-self.shift_size, None))
            cnt = 0
            for h in h_slices:
    ...
    def forward(self, x_main, x_mpmt):
        ...
        if self.shift_size > 0:
            def roll_2d(t):
                return torch.roll(t.view(B, H, W, C),
                                  shifts=(-self.shift_size, -self.shift_size), dims=(1, 2)).view(B, H*W, C)
        ...
        out = out.permute(0, 2, 1, 3).contiguous().view(out.size(0), -1, self.h * self.dh) 
        y = self._rev(out, H, W, C, self.ws)  # (B, N, C)

        if self.shift_size > 0:
            y = torch.roll(y.view(B, H, W, C), shifts=(self.shift_size, self.shift_size), dims=(1, 2)).view(B, N, C)
...
class SwinTransformerDI(nn.Module):
    def __init__(
        ...
        self.xattn_blocks = nn.ModuleList([
            CrossWindowBlock(dim=pre_stage_embed_dim,
                            input_resolution=tuple(self.patch_embed_main.patches_resolution),
                            window_size=pre_stage_window_size,
    ...
    def forward(self, x_main: torch.Tensor, x_mpmt38: torch.Tensor):
        ...
        num_patches = self.patch_embed_main.patches_resolution[0] * self.patch_embed_main.patches_resolution[1]
        mm = torch.zeros(B, num_patches, self.pre_stage_embed_dim, 
        ...
        if self.absolute_pos_embed is not None:
            ...
            B = x_main.size(0)
            num_patches = self.patch_embed_main.patches_resolution[0] * self.patch_embed_main.patches_resolution[1]
            occ = torch.zeros(B, num_patches, 1, device=mpmt_tokens.device, dtype=mpmt_tokens.dtype)
            b_idx = torch.arange(B, device=x_main.device)[:, None].expand_as(flat_indices)  # B×M
