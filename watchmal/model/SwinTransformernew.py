...
class AdvancedConvPatchEmbed(nn.Module):
   
    def __init__(self, img_size=(192, 192), patch_size=4, in_chans=2, embed_dim=96, norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        self.initial_proj = nn.Conv2d(in_chans, embed_dim, kernel_size=3, padding=1)

        self.res_blocks = nn.Sequential(
            PlainResBlock(channels=embed_dim),
            PlainResBlock(channels=embed_dim),
            PlainResBlock(channels=embed_dim)
        )
    ...
    def forward(self, x):
        ...
        x = x.unfold(2, PS_H, PS_H).unfold(3, PS_W, PS_W)
        x = x.permute(0, 2, 3, 1, 4, 5).contiguous()
        x = x.view(B * self.num_patches, C, PS_H, PS_W)
...
class SwinTransformerWithPreStage(nn.Module):
    def __init__(self, img_size=(192, 192), in_chans=2,
        ...
        super().__init__()
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=pre_stage_patch_size, in_chans=in_chans, 
...
class SwinTransformer(nn.Module):
    def __init__(self, img_size=(224, 224), patch_size=4, in_chans=3,
        ...
        self.patch_norm = patch_norm
        self.num_features = int(embed_dim * 2 ** (self.num_layers - 1))
        self.patch_embed = AdvancedConvPatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
